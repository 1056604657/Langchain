...
def fn_chat(
    ...
    if not messages:
        ...
        if stream:
            ...
            bot_response_generator = gpt.get_completion3(messages=messages, stream=True)
            for chunk in bot_response_generator:
                chat_history[-1][1] += chunk
                yield chat_history
            logger.success(f"流式输出完成 | bot_response: {chat_history[-1][1]}")
        else:
            bot_response = gpt.get_completion3(messages=messages)
            if isinstance(bot_response, str):
                chat_history[-1][1] = bot_response
            else:
                chat_history[-1][1] = str(bot_response)
            logger.success(f"非流式输出 | bot_response: {chat_history[-1][1]}")
            return chat_history
...
with gr.Blocks() as demo:
    gr.Markdown("# <centenr>超低排放问答</centenr>")
    # 定义一个行布局，内含两个等高的列布局
    with gr.Row(equal_height=True):
        # 左侧列布局：对话框
        with gr.Column(scale=4):
            chatbot = gr.Chatbot(label="聊天机器人")
            user_input_textbox = gr.Textbox(label="用户输入框", value="无组织排放有哪些方面？")
            with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            with gr.Tab(label="问答"):
                ...
                top_n_number = gr.Number(label="top_n", value=1)
            # 创建一个选项卡，用于调整参数
