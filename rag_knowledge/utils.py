...
def build_context_llama_index(qdrant, collection_names, question, top_n):
    scored_points = []
    for collection_name in collection_names:
        index = qdrant_index(qdrant.client, collection_name)
        ranker = RERANKERS.get("bge-reranker-base")
        retriever = index.as_query_engine(similarity_top_k=top_n, response_mode="tree_summarize", verbose=True,
                                          node_postprocessors=[ranker])
        query_bundle = QueryBundle(query_str=question)
        response = retriever.retrieve(query_bundle)
        #  response = ranker.postprocess_nodes(nodes=retriever_node, query_str=question)
        scored_points.extend(response)
...
def build_context_llama_index2(qdrant, engine_tools_params, question, top_n):
    scored_points = []
    engine_tools_last_params = []
    for engine_tools_param in engine_tools_params:
        index = qdrant_index(qdrant.client, engine_tools_param.collection_name)
        retriever = index.as_query_engine(similarity_top_k=top_n)
        engine_tools_last_params.append((retriever, engine_tools_param.metadata_name, engine_tools_param.description))
    query_engine_tools = get_query_engine_tools(engine_tools_last_params)
    query_engine = get_agent(query_engine_tools)
    response = query_engine.chat(question)

    # 构建上下文
    contexts = []
    for point in response.response:
    ...
    try:
        ...
        if file_paths and any(file_paths):
            for file_path in file_paths:
                file_processor = FileProcessor(file_path=file_path)
                file_name = file_processor.get_file_name()
                metadata_name_tmp = str(file_name.split("-")[0]).strip()
                metadata_name = str(metadata_name_tmp.split(".pdf")[0]).strip()
                ...
                engine_tools = EngineToolsParam(file_md5, metadata_name, f"提供了关于{metadata_name}的信息")
                engine_tools_params.append(engine_tools)
        ...
        top_n = int(top_n)
        # context = build_context_llama_index(
        #     qdrant,
        #     collection_names,
        #     user_input,
        #     top_n)
        context = build_context_llama_index(qdrant, collection_names, user_input, top_n)
